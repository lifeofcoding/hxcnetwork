...
export class ArtistInfo extends React.Component {
  ...
  getInfo(terms) {
    if (!this.isLoading) {
      this.isLoading = true;
      axios
        .get('/api/artists/' + encodeURIComponent(terms))
        .then(response => {
          // handle success
  ...
  render() {
    ...
    return (
      <div className="flex-container">
        <div
          className={isEmpty(artist) ? 'hide' : styles.wrapper}
          style={{opacity: this.state.opacity}}
        >
          <div
            className={'animated vanishIn ' + styles.flexContainer}
            style={{visibility: this.state.loaded ? 'visible' : 'hidden'}}
          >
            <div>
              <img src={artist.image} width="280px" height="auto" />
            </div>
            <div>
              <h3>{artist.name}</h3>i
              <p>
                <b>Genres:</b> {artist.genres}
              </p>
              <p>
                <b>Listeners:</b> {artist.listeners}
              </p>
              <div>
                <b>Related Artists:</b>{' '}
                <p>{this.renderRelated(artist.related)}</p>
              </div>
            </div>
          </div>
